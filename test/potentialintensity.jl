...
# @info cape_fortan_implementation, temp_outflow_fortran_implementation = get_cape(ustrip(tparcel),ustrip(rparcel), ustrip(pparcel), ustrip.(tabs), ustrip.(r), ustrip.(pres))
...
@testset "Potential Intensity" begin
    ...
    @test isapprox(cape, cape_fortran_implementation*u"J/kg", rtol = 0.01) broken=true
    @test isapprox(temp_outflow, temp_outflow_fortran_implementation*u"K", rtol = 0.01) broken=true
    @test isapprox(min_pres, min_pres_fortran_implementation*u"hPa", rtol = 0.01)
    @test isapprox(max_speed, max_speed_fortran_implementation*u"m/s", rtol = 0.01) broken=true
end